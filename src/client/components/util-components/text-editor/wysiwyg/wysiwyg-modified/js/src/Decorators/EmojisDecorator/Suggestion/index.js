      ...
      if (selection.get('anchorKey') === contentBlock.get('key') &&
        ...
        if (index >= 0) {
          ...
          getSuggestions().some((suggestion) => {
            if (suggestion.text) { //it was value
...
function getSuggestionComponent() {
  ...
  return class SuggestionComponent extends Component {
      ...
      this.filteredSuggestions =
        suggestions && suggestions.filter((suggestion) => {
          ...
          if (config.caseSensitive) {
            ...
            return suggestion.text.toLowerCase().indexOf(mentionText && mentionText.toLowerCase()) >= 0;
          }
        });
    }
    ...
    addEmoji = () => {
      ...
      addEmoji(editorState, onChange, separator, trigger, this.filteredSuggestions[activeOption]);
    }
